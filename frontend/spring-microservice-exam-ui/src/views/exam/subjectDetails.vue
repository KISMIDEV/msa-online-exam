<template>
  <div class="app-container">
    <el-tabs v-model="activeName" @tab-click="handleTabChange">
      <!-- 单选题 -->
      <el-tab-pane label="단답" name="0" :disabled="tempSubject.type !== 0 && dialogStatus !== dialogStatusType.create">
        <transition name="el-fade-in">
          <choices ref="choices" subjectInfo="tempSubject"/>
        </transition>
      </el-tab-pane>
      <!-- 多选题 -->
      <el-tab-pane label="다지선답" name="3" :disabled="tempSubject.type !== 3 && dialogStatus !== dialogStatusType.create">
        <transition name="el-fade-in">
          <multiple-choices ref="multipleChoices" subjectInfo="tempSubject"/>
        </transition>
      </el-tab-pane>
      <!-- 判断题 -->
      <el-tab-pane label="판단" name="2" :disabled="tempSubject.type !== 2 && dialogStatus !== dialogStatusType.create">
        <transition name="el-fade-in">
          <judgement ref="judgement" subjectInfo="tempSubject"/>
        </transition>
      </el-tab-pane>
      <!-- 简答题 -->
      <el-tab-pane label="객관" name="1" :disabled="tempSubject.type !== 1 && dialogStatus !== dialogStatusType.create">
        <transition name="el-fade-in">
      ...
      <el-tab-pane name="4" v-if="subjectsType === '0'" :disabled="dialogStatus !== dialogStatusType.create">
        <span slot="label"><i class="el-icon-document"/> 문제은행으로부터 추가</span>
        <transition name="el-fade-in">
