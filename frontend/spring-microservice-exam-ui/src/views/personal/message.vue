                  ...
                  <el-row>
                    <el-col :span="24">
                      <el-form-item :label="$t('table.born')" prop="born">
                        <el-date-picker v-model="userInfo.born" type="date" format="yyyy 년 MM 월 dd 일" value-format="timestamp" placeholder="出生日期"/>
                      </el-form-item>
                    </el-col>
                  </el-row>
                  <el-row>
...
export default {
  ...
  methods: {
    ...
    beforeAvatarUpload (file) {
      ...
      if (!isJPG) {
        this.$message.error('사진은 jpg/png 형식만 가능합니다!')
      }
      if (!isLt2M) {
        this.$message.error('사진크기는 2MB를 초과할수 없습니다!')
      }
      return isJPG && isLt2M
    }
  }
