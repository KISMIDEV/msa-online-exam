  ...
  <div class="app-container">
    <el-row>
      <el-col :span="4">
        ...
        <el-row>
          <el-button v-if="subject_category_btn_add" class="category-btn" icon="el-icon-plus" size="mini" type="primary" @click="handleAddSuperCategory">대분류</el-button>
          <el-button v-if="subject_category_btn_add" class="category-btn" icon="el-icon-plus" size="mini" type="primary" @click="handleAddCategory">소분류</el-button>
      ...
      <el-col :span="20">
        <el-card class="box-card">
          ...
          <div class="filter-container">
            <el-input v-model="listQuery.subjectName" placeholder="문제명" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
              ...
              <template slot-scope="scope">
                <el-dropdown>
                  <span class="el-dropdown-link">
                    조치<i class="el-icon-caret-bottom el-icon--right"></i>
                  </span>
      ...
      <el-row>
        <el-col :span="24">
          <el-upload
            ...
            <div class="el-upload__text">파일을 드래그하시거나 <em>여기를 클릭</em></div>
    ...
    <!-- 预览题目 -->
    <el-dialog title="문제프리뷰" :visible.sync="dialogViewVisible" width="60%" top="10vh">
      ...
      <ul v-if="tempSubject.type === 2" class="subject-options">
        <li class="subject-option">
          <input class="toggle" type="checkbox">
          <label><span class="subject-option-prefix">정답</span></label>
        </li>
        <li class="subject-option">
          <input class="toggle" type="checkbox">
          <label><span class="subject-option-prefix">오답</span></label>
        </li>
      </ul>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data () {
    return {
      ...
      // 表单校验规则
      categoryRules: {
        categoryName: [{ required: true, message: '분류명을 입력하세요', trigger: 'change' }]
      },
      // 表单校验规则
      subjectRules: {
        subjectName: [{ required: true, message: '문제명을 입력하세요', trigger: 'change' }],
        score: [{ required: true, message: '문제점수를 입력하세요', trigger: 'change' }],
        answer: [{ required: true, message: '답안을 입력하세요', trigger: 'change' }],
        serialNumber: [{ required: true, message: '문제일련번호를 입력하세요', trigger: 'change' }]
      },
      // 버튼权限
      ...
      tinymce: {
        type: 1, // 类型 0：문제명，1：选项A，2：选择B，3：选项C，4：选项D
