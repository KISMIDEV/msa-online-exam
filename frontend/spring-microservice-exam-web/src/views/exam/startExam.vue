    ...
    <el-row :gutter="30">
      <el-col :span="5">
        <div class="tool-bar">
          ...
          <el-button type="success" icon="el-icon-date" @click="submitExam" v-bind:disabled="disableSubmit">제출</el-button>
        </div>
      </el-col>
      <el-col :span="18">
        <div class="subject-box-card">
          <div class="subject-exam-title">{{exam.examinationName}}（모두{{subjectCount}}문제，합계{{exam.totalScore}}점）</div>
          <!-- 题目内容 -->
          ...
          <div class="subject-buttons" v-if="query.subjectId !== ''">
            <el-button plain @click="last" :loading="loadingLast">이전문제</el-button>
            <el-button plain @click="next" :loading="loadingNext">다음문제</el-button>
            <el-button type="success" @click="submitExam" v-bind:disabled="disableSubmit">제출</el-button>
          </div>
        </div>
      </el-col>
    </el-row>
    <el-dialog title="답안지" :visible.sync="dialogVisible" width="50%" top="10vh" center>
      <div class="answer-card-title" >{{exam.examinationName}}（모두{{subjectIds.length}}문제，합계{{exam.totalScore}}점）</div>
    ...
    validateExamTime () {
      getCurrentTime().then(response => {
        ...
        if (currentTime.isAfter(this.exam.endTime)) {
          messageWarn(this, '끝난 시험입니다. ')
        } else if (currentTime.isBefore(this.exam.startTime)) {
          messageWarn(this, '시험이 시작되기 전입니다.')
        } else {
          this.startExam()
      ...
      getSubjectIds(this.query.examinationId).then(subjectResponse => {
        ...
        if (subjectData.length > 0) {
          ...
          getSubjectAnswer({
            ...
            messageFail(this, '문제가져오기에 실패하였습니다！')
          })
        }
      }).catch(() => {
        messageFail(this, '시험을 시작하지 못했습니다！')
      })
    },
    countDownE_cb: function (x) {
      messageWarn(this, '시험종료')
      ...
      for (let i = 0; i < this.subjectIds.length; i++) {
        if (this.subjectIds[i].subjectId === this.query.subjectId) {
          if (i === this.subjectIds.length - 1) {
            messageSuccess(this, '마지막 문제 입니다.')
            break
          }
          let { subjectId, type, index } = this.subjectIds[++i]
          this.subjectIndex = index
    ...
    doSubmitExam (answer, examinationId, examRecordId, userInfo, toExamRecord) {
      ...
      saveAndNext(this.getAnswer(answerId), 0).then(response => {
        ...
        store.dispatch('SubmitExam', { examinationId, examRecordId, userId: userInfo.id }).then(() => {
          messageSuccess(this, '제출되었습니다.')
          if (toExamRecord) {
            this.$router.push({ name: 'exam-record' })
          }
        }).catch(() => {
          this.disableSubmit = false
          messageFail(this, '제출할수 없습니다.')
        })
      }).catch(() => {
        this.disableSubmit = false
        messageFail(this, '제출할수 없습니다.')
