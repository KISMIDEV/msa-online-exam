<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column prop="일련번호" label="id" min-width="100">
      ...
      <el-table-column label="첨부파일유형" min-width="90">
        <template slot-scope="scope">
          <span>{{ scope.row.busiType | attachmentTypeFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="첨부파일크기" min-width="90">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    handlePreview (row) {
      ...
      canPreview(row.id).then(response => {
        if (response.data.data) {
          ...
          messageWarn(this, '이 형식의 첨부 파일 미리보기는 현재 지원되지 않습니다')
        }
      }).catch(error => {
        console.error(error)
      })
    ...
    handleDownloadUrl (row) {
      ...
      this.$alert(url, '다운로드링크', { confirmButtonText: '확인' })
    },
    updateData () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
