      ...
      <el-col :span="24">
        <el-card class="score-info">
          ...
          <el-row>
            <el-col :span="6">
              <div class="user-info">
                <span class="user-info-item" :title="examRecord.examinationName">시험명：{{ examRecord.examinationName | simpleStrFilter }}</span>
                <br>
    ...
    getExamRecord () {
      examRecordDetails(this.examRecordId).then(response => {
        if (response.data.data === null) {
          messageFail(this, '성적로딩실패')
          return
        }
        this.examRecord = response.data.data
        const { inCorrectNumber, correctNumber, answers } = this.examRecord
        this.chartData.push({ name: '오답수', value: inCorrectNumber })
        this.chartData.push({ name: '정답수', value: correctNumber })
        this.list = answers
        this.initChart()
      }).catch(() => {
        messageFail(this, '성적로딩실패')
