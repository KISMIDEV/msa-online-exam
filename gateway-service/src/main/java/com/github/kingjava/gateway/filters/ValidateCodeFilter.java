package com.github.kingjava.gateway.filters;
...
import com.github.kingjava.common.core.exceptions.InvalidValidateCodeException;
import com.github.kingjava.common.core.exceptions.ValidateCodeExpiredException;
import com.github.kingjava.gateway.constants.GatewayConstant;
...
public class ValidateCodeFilter implements GlobalFilter, Ordered {
    ...
    private void checkCode(ServerHttpRequest serverHttpRequest, LoginTypeEnum loginType) throws InvalidValidateCodeException {
        ...
        if (StrUtil.isBlank(code))
            throw new InvalidValidateCodeException("보안코드를 입력하세요.");
        // 获取随机码
        ...
        if (!StrUtil.equals(saveCode, code)) {
            redisTemplate.delete(key);
            throw new InvalidValidateCodeException("보안코드오류.");
        }
        redisTemplate.delete(key);
    }
